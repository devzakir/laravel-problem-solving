  ...
  <div class="history__detail">
    ...
    <div class="total__part" v-if="!!order_form">
        ...
        <div class="calculator">
          ...
          <div class="calc__action" v-if="order_form.delivery == 1 && order_form.delivery_confirm == 0">
            <button @click="deliveryConfirmProc" class="on">受取連絡</button>
          </div>
          <!-- <div class="calc__action black">
            <button><img src="/asset/icons/output2.svg" />再発注</button>
          </div> -->
...
export default {
  ...
  methods: {
    ...
    async okBankPayment() {
      this.isShowModal = false
      try {
        const { data } = await axios.post('/api/delivery_confirm_proc', {
          id: this.$route.query.id
        })
        this.$swal('', '受取完了連絡を送りました。')
