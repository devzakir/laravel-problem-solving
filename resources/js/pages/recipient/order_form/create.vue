<template>
  <div class="form__container">
    <h2 class="page__title">注文フォーム</h2>
    <div class="form__part">
      <div class="form__row">
        <label>フォーム名<span class="must">必須</span></label>
        <p>
          <input type="text" v-model="name" placeholder="〇〇〇〇の注文フォーム" />
        </p>
      </div>
      <!-- <div class="form__row">
        <label>URL</label>
        <p>
          Https://conelce.com/165asdfa
        </p>
      </div> -->
      <div class="form__row area">
        <label>お取引条件</label>
        <p>
          <textarea v-model="condition" placeholder="例）締切時間、送料、休業日、キャンペーン期間など">
          </textarea>
        </p>
      </div>
      <div class="form__row">
...
export default {
  ...
  methods: {
    ...
    async save() {
      ...
      try {
        const { data } = await axios.post('/recipienter/create_order_form', {
          name: this.name,
