<template>
  <div class="main__container">
    <h2 class="page__title">メール配信履歴</h2>
    ...
    <div class="table__header">
      ...
      <div class="exort__part">
        <!-- <a class="append_btn"><img src="/asset/icons/append.svg" />受注登録</a> -->
      ...
      <vue-good-table
        ...
        <template slot="table-row" slot-scope="props">
          <div v-if="props.column.field == 'detail'">
            <button @click="toDetailPage(props.row.id)" class="detail_btn">詳細</button>
          </div>
          <div v-else-if="props.column.field == 'target'">
            {{ props.row.user.com_name }}
          </div>
          <div v-else-if="props.column.field == 'created_at'">
            {{ props.row.created_at | dateFormatFull }}
          </div>
...
export default {
  ...
  methods: {
    ...
    toDetailPage(id) {
      this.$router.push({ name: 'recipient.message.detail', query: { id: id } })
    },
    async init() {
      try {
        const { data } = await axios.post('/recipienter/get_mail_histories')
        this.tempRows = data.histories
        this.rows = this.tempRows.filter((item, index) => {
          return index < 10
        })
        this.setPagination()
