  ...
  <div class="main__container">
    ...
    <div class="table__header">
        ...
        <div class="d-flex">
          <div class="form__row d-flex">
            ...
            <select v-model="status">
              ...
              <option :value="2">出荷伝票作成済み</option>
              <option :value="3">出荷伝票未作成</option>
            </select>
          </div>
          <div class="form__row d-flex">
            <label>納品希望日：</label>
      ...
      <div class="exort__part">
        <router-link :to="{ name: 'recipient.order1.create' }" class="append_btn"><img src="/asset/icons/append.svg" />受注登録</router-link>
        <div class="download_panel">
          <a @click="finishedDelivery"><img src="/asset/icons/slip.svg" />伝票作成</a>
          <a @click="generatePDF"><img src="/asset/icons/pdf.svg" />明細</a>
          <a @click="generateCSV"><img src="/asset/icons/csv.svg" />明細</a>
        </div>
    ...
    <div class="table__container">
      <vue-good-table
        ...
        <template slot="table-row" slot-scope="props">
          ...
          <div v-else-if="props.column.field == 'shipping_slip'">
            <span class="shipping_slip" :class="{ off: !props.row.delivery_card == 1 }">{{ props.row.delivery_card == 1 ? '作成済' : '未作成' }}</span>
          </div>
          ...
          <div v-else-if="props.column.field == 'delete'">
            <a @click="deleteOrder(props.row.id)" class="delete_btn"><img src="/asset/icons/delete.svg" /></a>
          </div>
    ...
    <ConfirmModal 
      ...
      @ok="finishMultiDelivery"/>

    <ConfirmModal 
      v-if="confirmPDF" 
      :note="'出荷伝票をダウンロードしますか？'" 
      :cancelText="'キャンセル'" 
      :okText="'決定'"
      @cancel="() => { confirmPDF = false }"
      @ok="generatePDFConfirm"/>
...
export default {
  ...
  methods: {
    ...
    async deleteOrderConfirm() {
      ...
      try {
        const { data } = await axios.post('/recipienter/delete_order', {
          id: this.tempOrderId
        })
        this.init()
      } catch (error) {
    ...
    async search() {
      try {
        const { data } = await axios.post('/recipienter/get_orders_list')
    ...
    async finishMultiDelivery() {
      this.isShowConfirm = false
      try {
        const { data } = await axios.post('/recipienter/finish_multi_delivery', {
          ids: this.tempRes
        })
        this.init()
      } catch (error) {
    ...
    async finishedDelivery() {
      let filter = this.rows.filter(item => {
        return item.selected == true
      })
